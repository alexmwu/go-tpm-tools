...
func TestParseEventLogs(t *testing.T) {
	...
	logs := []struct {
		...
		{Ubuntu2104NoDbxGCE, "Ubuntu2104NoDbxGCE", ParseOpts{Loader: GRUB}, sbatErrorStr},
		{Ubuntu2104NoSecureBootGCE, "Ubuntu2104NoSecureBootGCE", ParseOpts{Loader: GRUB}, sbatErrorStr},
		// This event log has a SecureBoot variable length of 0.
		{ArchLinuxWorkstation, "ArchLinuxWorkstation", ParseOpts{}, archLinuxBadSecureBoot},
		{COS85AmdSev, "COS85AmdSev", ParseOpts{Loader: GRUB}, ""},
		{COS93AmdSev, "COS93AmdSev", ParseOpts{Loader: GRUB}, ""},
	}
...
func TestParseMachineStateReplayFail(t *testing.T) {
	...
	_, err := ParseMachineState(Debian10GCE.RawLog, &badPcrs, ParseOpts{})
	if err == nil {
...
func TestParseSecureBootState(t *testing.T) {
	for _, bank := range UbuntuAmdSevGCE.Banks {
		msState, err := ParseMachineState(UbuntuAmdSevGCE.RawLog, bank, ParseOpts{})
		if err != nil {
			t.Errorf("failed to parse and replay log: %v", err)
		}
...
func TestParseGrubState(t *testing.T) {
	...
	for _, log := range logs {
		for _, bank := range log.Banks {
			...
			t.Run(subtestName, func(t *testing.T) {
				msState, err := ParseMachineState(log.RawLog, bank, ParseOpts{Loader: GRUB})
				if err != nil {
					t.Errorf("failed to parse and replay log: %v", err)
				}
...
func TestParseGrubStateFail(t *testing.T) {
	...
	for _, bank := range eventlog.Banks {
		...
		t.Run(subtestName, func(t *testing.T) {
			_, err := ParseMachineState(eventlog.RawLog, bank, ParseOpts{Loader: GRUB})
			if err == nil {
